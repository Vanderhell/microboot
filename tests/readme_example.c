...
static uint32_t readme_clock(void)
{
    return 100u;
}
...
{
    readme_backend_t *backend = (readme_backend_t *)ctx;
...
int main(void)
{
    ...
    err = mboot_init(&boot, &io, readme_clock, &config);
    if (err != MBOOT_OK) {
        return 1;
    }
    ...
    if (mboot_mode(&boot) != MBOOT_MODE_NORMAL) {
        return 3;
    }
